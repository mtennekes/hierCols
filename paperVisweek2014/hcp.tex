...
Three visualisation methods were used in the survey, namely the (directed) graph, the treemap and the bar plot. For each of the three methods, respondents received questions of two plots of two different datasets, one with Tree Colors and one with qualitative that are assinged to the first hierarchical layer. The latter palettes are called First Colors. The colors are taken from ColorBrewer palettes~\cite{brewer03}, which are quite popular in cartography and statistical visualizations.


In order to exclude the effects of all other aestetics than color as much as possible, we spread two versions of the survey where per visualization method the datasets were swapped. Therefore, the same visualization is shown to both groups with different color schemes, which is illustrated in Figures~\ref{fig:graphSvy},~\ref{fig:treemapSvy}, and~\ref{fig:barSvy}. An overview of the plots used in the two versions of the questionnaires is provided in Table~\ref{table:ques}.
...
\end{tabular}
\end{footnotesize}
\caption{Questionairre setup}\label{table:ques}
\end{table}
