...
In order to show depth, we let $C$ and $L$ values only depend on the depth of the corresponding nodes in the tree structure. We let the $L$ decrease linearly with depth and $C$ increase: having more intense colors helps in discriminating leaf nodes. For each hierarchical level $i \in \{0, 1, \ldots, k\}$, where 0 is the root, the chroma and luminance values are given by
\begin{equation}
C_i=i\beta^C + C_0
\end{equation}
and
...
By default, the parameters are set as follows: $\beta^C=5$, $\beta^L=-10$, $C_0=65$ and $L_0=80$. Since the root node itself does not have a suitable hue value, its color is specified by $[H=0, C=0, L=L_0]$.

% LITTLE CONFLICT WITH TREEMAPs palette.HCL.options: they start with the level=1, while the formulation above is more elegant.
