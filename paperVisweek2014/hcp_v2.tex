...
Such color schemes could clarify the chosen layout by emphasizing the tree structure of the data. For node-link diagrams, this makes it possible to relax \changedE{its} tree layout, having more space to place the nodes and labels.
...
We evaluated the Tree Colors method with a user study by comparing it to a color scheme with a distinct colors for each main branch of the tree. The results of this study \changedE{indicate} %provided evidence
that Tree Colors can be effective in tree visualizations, especially in tree structured node-link diagrams. However, the results \changedE{for treemaps} %regarding two dimensional visualizations 
...
\cite{newsmap} aggregates news and presents a tree map of news to its users. 
\changedE{The color of each news item indicates one of seven news types. Although a treemap the newsmap.jp color scheme is not hierarchical. Each news type has a fixed hue value, and the colors within a news type differ in chroma and luminance to make the rectangles more distinct and attractive. In contrast when using Tree Colors sibling nodes have different hues but equal luminance and chroma}.
IBM's Many Eyes \cite{manyeyes} \changedE{uses HSV color schemes for treemaps that take groups into account. The colors vary in hue but all have equal saturation and value. Nodes within a group have similar hue values. This is comparable to Tree Colors, but the Many Eyes color scheme is restricted to one grouping and thus not hierarchical. Furthermore  instead of HSV Tree Colors uses the perception based HCL color space model, keeping Chroma and Luminance for sibling nodes equal.
}
...
%\E : \textbf{comparison with Tree Colors}
...
The permutations and reversals of hue ranges is developed particularly for visualization methods in which sibling nodes are arranged linearly or radially. However, when sibling nodes are arranged differently, for instance in treemaps, the coloring of sibing nodes may appear random. Therefore, adjacent nodes might get indistinguishable colors in such visualizations.
...
Although Tree Colors were not developed with color blindness in mind, it is important to know whether Tree Colors \changedE{are} %could be 
perceived adequately by people with a color vision deficiency, \changedE{because that is quite common}%because there are many such people
~\cite{birch12}.

People with normal color vision, called trichromates, perceive colors by three classes of cone opsins, the \changedE{S-, M- }%L-, S-, 
...
Figure~\ref{fig:sunburst} shows the net turnover of the Dutch wholesale and retail trade enterprises in 2011~\cite{cbsSBS} in a sunburst layout~\cite{stasko2000evaluation, stasko2000focus+} with Tree Colors. The angle \changedE{of a arc} corresponds to \changedE{its} net turnover. \changedE{A sunburst plot, which is a radial version of a layered icicle plot~\cite{kruskal1983icicle,burch2010indented}, typically} repeats the same qualitative color scheme for child nodes: sibling nodes have different colors, but their children use the same color scheme. This makes siblings more distinct, \changedE{but makes the tree structure less visible}. In contrast, Tree Colors better show the \changedE{hierarchical} structure of data.
...
\begin{figure}[!b]
  ...
  \caption{Sunburst diagram with Tree Colors.\E \textbf{: adjust labels}}\label{fig:sunburst}
\end{figure}
...
\begin{figure}[!t]
  ...
  \caption{Turnover among Dutch wholesale and retail trade enterprises in 2011 \M \textbf{: check colors and sizes (consistancy with other plots}}\label{fig:treemapApp}
\end{figure}
...
The three evaluation questions are summarized in Figure~\ref{fig:user2} with a diverging stacked bar chart. In general, the participants liked both color schemes equally, although Tree Colors were favored for node-link diagrams.

The charts with Main Branch Colors provided the best overview for the majority of the participants. This result is not surprising, since a good overview is obtained by discriminating the main branches. Recall that Tree Colors can be adjusted with parameter $f$ in order to better discriminate main branches, and therefore getting a better overview at the expense of the discrimintaion of leaf nodes.
